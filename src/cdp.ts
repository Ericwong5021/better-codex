...
async function bridgeRequest(connection: Connection, runtimePort: number, accessToken: string, payload: unknown) {
  ...
  try {
    ...
    if (!requestId || request.token !== accessToken || !/^\/api\/(?:bootstrap(?:[?]|$)|update(?:\/(?:install|check))?(?:[?]|$)|projects(?:\/ensure)?(?:[?]|$)|issues(?:[/?]|$)|agents(?:[/?]|$)|settings\/auto-dispatch(?:[?]|$))/.test(path) || !["GET", "POST", "PATCH", "DELETE"].includes(method)) throw new Error("invalid_bridge_request");
