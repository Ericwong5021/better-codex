...
async function bridgeRequest(connection: Connection, runtimePort: number, accessToken: string, payload: unknown) {
  ...
  try {
    ...
    if (!requestId || request.token !== accessToken || !/^\/api\/(?:bootstrap(?:[?]|$)|update(?:\/(?:install|check))?(?:[?]|$)|projects(?:\/ensure)?(?:[?]|$)|issues(?:[/?]|$)|sessions(?:[/?]|$)|session-relay(?:[/?]|$)|agents(?:[/?]|$)|workflows(?:[/?]|$)|workflow-runs(?:[/?]|$)|publishers(?:[?]|$)|mockup\/(?:state|reset)(?:[?]|$)|settings\/auto-dispatch(?:[?]|$)|settings\/scheduler-model(?:[?]|$)|settings\/scheduler-reasoning-effort(?:[?]|$))/.test(path) || !["GET", "POST", "PUT", "PATCH", "DELETE"].includes(method)) throw new Error("invalid_bridge_request");
