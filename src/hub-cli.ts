...
if (command === "serve") {
  ...
  try {
    ...
    else throw new Error("usage: better-codex-hub serve|pairing-code|devices|revoke <device-id>|clear-projection|password-set|backup [path]|restore <path>|audit [limit]");
  } finally {
    store.close();
  }
}
