...
import { cdpEject, cdpInject, cdpOpenThread, cdpStatus, watchInjection } from "./cdp.js";
import {
  cdpPort,
  ensureDirectories,
  gatewayLogPath,
...
function spawnSelf(args: string[], logFile: string) {
  ...
  const child = spawn(process.execPath, ownArgs, {
    cwd: process.cwd(),
    detached: true,
    env: { ...process.env, TILO_TOKEN: accessToken() },
...
function usage() {
  console.log("tilo version | start [--launch] | stop | status | inject [--launch] [--port N] | eject [--port N] | service install|uninstall|start|stop|restart|status|logs | project list|create | issue list|get|create|update|status|link|open");
}
...
async function main() {
  ...
  if (command === "version" || command === "--version" || command === "-v") return console.log("tilo 0.1.0");
...
if (process.argv[2] === "serve") console.log(`Tilo home: ${tiloHome}`);
