...
export class Store {
  ...
  async applyRemoteCommand(command: RemoteCommand, handlers: { files?: (files: Array<{ name: string; type: string; data: string }>) => { paths: string[]; cleanup: () => void } | Promise<{ paths: string[]; cleanup: () => void }>; reply?: (issueId: string, requestId: string, message: string, files: Array<{ name: string; type: string; data: string }>) => void | Promise<void>; queue?: (issueId: string, requestId: string, action: "update" | "send" | "delete", message?: string) => void | Promise<void>; stop?: (issueId: string) => void | Promise<void>; regenerateTitle?: (issueId: string) => void | Promise<void>; projectCreate?: (projectId: string, name: string, workspacePath: string) => void | Promise<void>; projectDelete?: (projectId: string) => void | Promise<void>; projectOverview?: (projectId: string, agentId: string, feedback: string) => void | Promise<void>; projectPlanningReply?: (projectId: string, agentId: string, message: string) => void | Promise<void>; projectPlanningReset?: (projectId: string) => void | Promise<void>; chooseDirectory?: () => string | Promise<string>; browseDirectory?: (path: string) => DirectoryBrowserResult | Promise<DirectoryBrowserResult>; createDirectory?: (parentPath: string, name: string) => string | Promise<string>; threadAction?: (issueId: string, action: IssueThreadAction) => void | Promise<void> } = {}): Promise<RemoteCommandAck> {
    ...
    const result = await (async () => {
      ...
      try {
        ...
        if (command.operation === "project.create_directory") {
          if (command.base_revision !== null || !handlers.createDirectory || typeof payload.parent_path !== "string" || typeof payload.name !== "string") throw new Error("directory_creation_unavailable");
          const workspacePath = await handlers.createDirectory(payload.parent_path, payload.name);
          return { command_id: command.command_id, status: "applied", error: null, projection: null, result: { workspace_path: workspacePath } } satisfies RemoteCommandAck;
        }
        ...
        this.db.prepare(`
          ...
          WHERE id = ?
            AND (
              ...
              OR (status = 'in_review' AND needs_attention = 0 AND pending_actor = 'agent')
            )
            AND NOT EXISTS (
              SELECT 1 FROM issue_runs
              WHERE issue_runs.issue_id = issues.id
    ...
    try {
      const run = this.db.prepare(`
        ...
        WHERE interrupted.issue_id = ?
          ...
          AND issues.status IN ('blocked', 'in_review')
          AND interrupted.id = (
            SELECT latest.id
      ...
      const issueResult = this.db.prepare(`
        ...
        SET status = 'in_review',
            ...
            updated_at = ?
        WHERE id = ?
          AND status IN ('blocked', 'in_review')
          AND ? = (
            SELECT latest.id
            FROM issue_runs AS latest
  ...
  syncSessionReply(issueId: string, threadId: string, activity: {
    ...
    try {
      ...
      if (activity.status === "interrupted" && reply?.status === "interrupted" && ["in_progress", "in_review", "blocked"].includes(issue.status)) {
        this.db.exec("COMMIT");
        return false;
      }
      ...
      const unchangedRuntimeInterruption = reply?.status === "interrupted"
        ...
        && ["in_review", "blocked"].includes(issue.status)
        && replyFinishedTime > 0
  ...
  transitionUpdateOperation(id: string, status: UpdateOperationStatus, patch: {
    ...
    const transitions: Record<UpdateOperationStatus, UpdateOperationStatus[]> = {
      ...
      DRAINING_DISPATCH: ["WAITING_FOR_HOST_DRAIN", "HANDOFF_READY", "ROLLING_BACK", "FAILED"],
      WAITING_FOR_HOST_DRAIN: ["HANDOFF_READY", "ROLLING_BACK", "FAILED"],
      HANDOFF_READY: ["RESTARTING_RUNTIME", "ROLLING_BACK", "FAILED"],
  ...
  failClaimedSessionCommands(relayId?: string, owner: "host" | "native" = "host") {
    ...
    for (const row of rows) {
      const command = sessionCommandFromRow(row);
      if (command.kind === "rename" && command.attempts < 5) {
        this.db.prepare(`
          UPDATE session_commands
      ...
      for (const row of rows) {
        ...
        if (command.kind === "rename") {
          this.db.prepare(`
            ...
            WHERE id = ? AND status = 'claimed'
          `).run(command.id);
          reconciled.push({ ...command, status: "pending", relay_id: null, claimed_at: null, error: null, finished_at: null });
          continue;
        }
    ...
    return this.transaction(() => {
      ...
      if ((command.kind === "start" || command.kind === "turn" || command.kind === "review") && turnId) {
        this.db.prepare(`
          ...
          WHERE id = ? AND status != 'done' AND archived_at IS NULL
            AND (status != 'in_progress' OR needs_attention != 0 OR pending_actor != 'agent')
        `).run(timestamp, command.issue_id);
      }
      return this.getSessionCommand(commandId)!;
    });
  ...
  failSessionCommand(commandId: string, relayId: string, error: string, partialThreadId?: string | null, partialTurnId?: string | null) {
    ...
    return this.transaction(() => {
      ...
      } else if (command.kind !== "start" && command.kind !== "rename") {
