...
export function relayDiagnostic(event: string, detail: Record<string, unknown>) {
...
const missingThread = sessionThreadMissing;
...
export class AppServerSessionWorker {
  ...
  private readonly allocatedThreads = new Map<string, string | null>();
  private readonly retainedThreads = new Set<string>();
  private readonly residentThreadResults = new Map<string, Record<string, unknown>>();
  ...
  status() {
    return {
      ...
      command_in_flight: this.commandInFlight || this.queuedCommandCount > 0,
      ...
      awaiting_persistence: [...this.allocatedThreads.keys()].filter(threadId => !this.threadIsDurable(threadId)),
  ...
  idle() {
    return this.queuedCommandCount === 0 && !this.commandInFlight && this.pending.size === 0 && this.activeTurns.size === 0;
  }
  ...
  threadIsDurable(threadId: string) {
    if (!this.allocatedThreads.has(threadId)) return true;
    const path = this.allocatedThreads.get(threadId);
  ...
  retainUntilDurable(threadId: string, reason: string) {
    if (this.threadIsDurable(threadId)) return false;
    if (!this.retainedThreads.has(threadId)) {
      this.retainedThreads.add(threadId);
      relayDiagnostic("thread_worker_retained", { host_instance_id: this.hostInstanceId, thread_id: threadId, app_server_pid: this.child?.pid, reason, persistence: "awaiting_first_turn" });
    }
    return true;
  }
  ...
  busyForThread(threadId: string) {
    return (this.queuedCommandCount > 0 && this.ownsThread(threadId)) || (this.commandInFlight && this.currentThreadId === threadId) || this.activeTurns.has(threadId) || this.busyThreads.has(threadId) || (this.pending.size > 0 && this.ownsThread(threadId));
  }
  ...
  private disconnect(child: ChildProcessWithoutNullStreams, error: string) {
    ...
    this.activeTurns.clear();
    this.busyThreads.clear();
    this.allocatedThreads.clear();
    this.retainedThreads.clear();
    this.residentThreadResults.clear();
    if (this.heartbeatTimer) clearInterval(this.heartbeatTimer);
    this.heartbeatTimer = null;
  ...
  private heartbeat(relayId: string) {
    ...
    void Promise.resolve().then(() => this.host.poll(relayId, true)).catch(error => {
      relayDiagnostic("session_command_heartbeat_failed", { host_instance_id: this.hostInstanceId, relay_id: relayId, app_server_pid: this.child?.pid, thread_id: this.currentThreadId || null, error: error instanceof Error ? error.message : String(error) });
    });
  }
  ...
  execute(command: SessionCommand, relayId: string) {
    ...
    return this.serialize(async () => {
      ...
      return this.executeCommand(command, relayId);
  ...
  private async executeCommand(command: SessionCommand, relayId: string) {
    ...
    try {
      ...
      if (command.kind === "bind" || command.kind === "start") {
        ...
        threadId = sessionId(thread.id);
        if (!threadId) throw new Error("desktop_thread_start_invalid");
        this.residentThreadResults.set(threadId, started);
        this.allocatedThreads.set(threadId, typeof thread.path === "string" && thread.path ? thread.path : null);
        relayDiagnostic("thread_allocated", { command_id: command.id, issue_id: command.issue_id, thread_id: threadId, host_instance_id: this.hostInstanceId, app_server_pid: this.child?.pid, app_server_version: this.appServerVersion, persistence: this.threadIsDurable(threadId) ? "durable" : "awaiting_first_turn" });
        this.currentThreadId = threadId;
        this.threads.add(threadId);
        this.options.onThreadBound?.(threadId, this);
        await this.host.checkpoint(command.id, relayId, { thread_id: threadId });
        if (command.kind === "start") {
          ...
          await this.host.checkpoint(command.id, relayId, { thread_id: threadId, turn_id: turnId });
        }
      ...
      await this.host.complete(command.id, relayId, { thread_id: threadId, turn_id: turnId, persistence: this.threadIsDurable(threadId) ? "durable" : "awaiting_first_turn", ...completion });
      this.commandInFlight = false;
      ...
      relayDiagnostic("session_command_failed", { command_id: command.id, issue_id: command.issue_id, kind: command.kind, host_instance_id: this.hostInstanceId, app_server_pid: this.child?.pid, app_server_version: this.appServerVersion, thread_id: threadId || null, turn_id: turnId || null, phase: turnId ? "turn_started" : threadId ? "thread_bound" : "thread_allocation", error: commandError });
      ...
      try {
        await this.host.fail(command.id, relayId, commandError, threadId || undefined, turnId || undefined);
      } catch (deliveryError) {
        relayDiagnostic("session_command_failure_delivery_failed", { command_id: command.id, thread_id: threadId || null, host_instance_id: this.hostInstanceId, command_error: commandError, error: deliveryError instanceof Error ? deliveryError.message : String(deliveryError) });
        throw deliveryError;
      }
      this.commandInFlight = false;
      this.flush("", true);
  ...
  private async resume(threadId: string, payload?: Record<string, unknown>) {
    const resident = this.residentThreadResults.get(threadId);
