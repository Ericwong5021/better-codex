...
export class SyncClient {
  ...
  constructor(
    ...
    private readonly createDirectory: (parentPath: string, name: string) => string | Promise<string> = () => { throw new Error("directory_creation_unavailable"); },
  ...
  private async pullCommands(configuration: SyncConfiguration) {
    for (let page = 0; page < 100; page += 1) {
      ...
      for (const command of result.commands) {
        const ack = { ...(await this.store.applyRemoteCommand(command, { files: this.files, reply: this.reply, queue: this.queue, stop: this.stopIssue, regenerateTitle: this.regenerateTitle, projectCreate: this.projectCreate, projectDelete: this.projectDelete, projectOverview: this.projectOverview, projectPlanningReply: this.projectPlanningReply, projectPlanningReset: this.projectPlanningReset, chooseDirectory: this.chooseDirectory, browseDirectory: this.browseDirectory, createDirectory: this.createDirectory, threadAction: this.threadAction })), delivery_id: command.delivery_id ?? null };
