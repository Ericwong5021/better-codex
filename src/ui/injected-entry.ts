...
export function install(config: Record<string, any>) {
  if (!config || config.schemaVersion !== 1) throw new Error("injected_ui_schema_mismatch");
    ...
    function projectDashboardMarkup(project, issues, issuesLoading, paths) {
      ...
      const planningPane = state.projectPlanningPane === "chat" ? "chat" : "plan";
      const desktopTabs = '<nav class="better-codex-project-dashboard-tabs better-codex-project-dashboard-tabs-desktop" aria-label="' + te("项目页面") + '"><button type="button" data-project-dashboard-view="overview"' + (page === "overview" ? ' aria-current="page"' : "") + '>' + te("概览") + '</button><button type="button" data-project-dashboard-view="planning"' + (page === "planning" ? ' aria-current="page"' : "") + '>' + te("规划") + '</button><button type="button" data-project-dashboard-view="work"' + (page === "work" ? ' aria-current="page"' : "") + '>' + te("工作") + '</button></nav>';
      const mobileTabs = '<nav class="better-codex-project-dashboard-tabs better-codex-project-dashboard-tabs-mobile" aria-label="' + te("项目页面") + '"><button type="button" data-project-dashboard-view="overview"' + (page === "overview" ? ' aria-current="page"' : "") + '>' + te("概览") + '</button><button type="button" data-project-dashboard-view="planning" data-project-planning-pane="plan"' + (page === "planning" && planningPane === "plan" ? ' aria-current="page"' : "") + '>' + te("规划") + '</button><button type="button" data-project-dashboard-view="planning" data-project-planning-pane="chat" aria-label="' + te("规划对话") + '"' + (page === "planning" && planningPane === "chat" ? ' aria-current="page"' : "") + '>' + te("对话") + '</button><button type="button" data-project-dashboard-view="work"' + (page === "work" ? ' aria-current="page"' : "") + '>' + te("工作") + '</button></nav>';
      const header = '<header class="better-codex-project-dashboard-head"><div class="better-codex-project-dashboard-title"><div><h1>' + escapeHtml(projectLabel(project)) + '</h1><span data-project-health-badge data-tone="' + healthTone + '">' + escapeHtml(healthLabel) + '</span></div><p>' + escapeHtml(project.description || t("尚未生成项目介绍")) + '</p></div>' + headerActions + '</header>' + desktopTabs + mobileTabs;
    ...
    async function openProjectDetail(projectId) {
      ...
      state.projectPlanningPane = "plan";
    ...
    function onProjectsClick(event) {
      ...
      if (dashboardView) {
        ...
        if (state.projectPage === "planning") state.projectPlanningPane = dashboardView.dataset.projectPlanningPane === "chat" ? "chat" : "plan";
        renderProjects();
        if (state.projectPage === "planning" && (state.projectPlanningPane === "chat" || !window.matchMedia("(max-width: 980px)").matches)) requestAnimationFrame(() => {
