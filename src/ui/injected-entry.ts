...
export function install(config: Record<string, any>) {
  if (!config || config.schemaVersion !== 1) throw new Error("injected_ui_schema_mismatch");
    ...
    const localeResources = { "zh-CN": {}, en: {
      ...
      "负责人": "Owner", "创建人": "Creator", "创建者": "Creator", "未知创建人": "Unknown creator", "指定负责人": "Assign owner", "由我创建": "Created by me", "由我": "By me", "我": "Me", "你": "You", "未指派": "Not assigned", "未提供": "Not provided", "已同步": "Synced",
      "对话已被占用": "Conversation is in use",
      "这个对话已在 Codex CLI 或其他 Codex 客户端中打开。请先在 CLI 或对应客户端关闭这个对话，再返回这里重新打开。": "This conversation is open in Codex CLI or another Codex client. Close it in that client, then return here and try opening it again.",
      ...
      "确定删除任务 “": "Delete task “", "确定删除所有已归档任务吗？": "Delete all archived tasks?", "吗？": "”?", "刚刚": "Just now", "分钟": "minutes", "小时": "hours", "天": "days", "更新于": "Updated", "个筛选": "filters", "个智能体工作中": "agents working", "条": "items",
      ...
      "工作中": "Working", "排队中": "Queued", "执行失败": "Execution failed", "已停止": "Stopped", "未开始": "Not started", "无法连接 Better Codex Runtime": "Unable to connect to Better Codex Runtime",
    ...
    Object.assign(localeResources.en, {
      ...
      "当前通道暂无新版本": "No newer version in this channel",
      "此通道仅检查正式版，不会提示 Beta 更新。": "This channel checks stable releases only. Beta updates are not included.",
      "前往网站管理更新": "Manage updates on the website",
    ...
    function t(value) {
      ...
      match = core.match(/^创建后由 (.+) 立即开始工作，同时在后台生成 Issue 标题。$/);
      if (match) return leading + "After creation, " + match[1] + " will start working immediately while generating the Issue title in the background." + trailing;
    ...
    function queuedIssuePlaceholder(commandId, body) {
      ...
      return {
        ...
        creator_user_id: state.user.id || null,
        version: 1,
        reply_status: "idle",
    ...
    function confirmAction(title, message, confirmLabel = "确认", notice = false) {
      ...
      return new Promise(resolve => {
        ...
        const accept = createButton({ label: t(confirmLabel), onPress: () => finish(true), variant: notice ? "primary" : "danger" }, componentContext("confirmation", "confirm-accept:" + (++managedButtonSequence)));
        accept.element.classList.add("better-codex-confirm-primary");
        accept.element.dataset.confirmAccept = "true";
        if (!notice) actions.append(cancel.element);
        actions.append(accept.element);
    ...
    function openFilterMenu(trigger) {
      ...
      const categories = [
        ...
        { key: "creator", text: "创建人", icon: "userEdit" },
    ...
    async function openProjectDetail(projectId) {
      ...
      controllers().projects.resetPlanningPane();
    ...
    function onProjectsClick(event) {
      ...
      if (dashboardView) {
        ...
        const selection = controllers().projects.selectDashboard(page, dashboardView.dataset.projectPlanningPane || "");
        state.projectPage = selection.page;
        renderProjects();
        if (state.projectPage === "planning" && (selection.planningPane === "chat" || !window.matchMedia("(max-width: 980px)").matches)) requestAnimationFrame(() => {
    ...
    function renderSettingsOverlay(initialView = "mode") {
      ...
      const remoteUpdateSummary = dialog.querySelector("[data-remote-update-summary]");
      const remoteUpdateChannel = dialog.querySelector("[data-remote-update-channel]");
      const remoteUpdateSummaryText = dialog.querySelector("[data-remote-update-summary-text]");
      ...
      const renderRemoteUpgrade = (update, visible) => {
        ...
        remoteUpdateSummary.hidden = !visible || !update;
        if (update) {
          remoteUpdateChannel.textContent = t("更新通道") + " · " + (update.channel === "preview" ? t("预览版") : update.channel === "stable" ? t("稳定版") : "--");
          const channelNotice = update.channel === "stable" && String(update.currentVersion || "").includes("-beta.");
          remoteUpdateSummaryText.textContent = update.status === "available" ? t("发现可用升级") + " v" + String(update.latestVersion || "").replace(/^v/, "") + (REMOTE ? "" : " · " + t("前往网站管理更新")) : update.status === "error" ? updateErrorLabel(new Error(update.error || "update_check_failed")) : channelNotice ? t("此通道仅检查正式版，不会提示 Beta 更新。") : !update.checkedAt ? t("尚未检查更新") : update.status === "current" ? t("当前通道暂无新版本") : t("正在更新");
        }
        ...
        const stage = String(update?.status === "available" && !remoteUpdateActive ? "available" : update?.stage || (installing ? "preparing" : remoteUpdateActive ? "checking" : update?.status === "error" ? "error" : ""));
        ...
        const signature = [update?.channel || "", update?.status || "", stage, Math.round(progress), update?.currentVersion || "", update?.latestVersion || "", update?.error || ""].join(":");
        if (signature !== remoteUpdateSignature) {
          remoteUpdateSignature = signature;
          appendDiagnostic("remote_update_state", { channel: update?.channel || "", status: update?.status || "", stage, progress: Math.round(progress), current_version: update?.currentVersion || "", target_version: update?.latestVersion || "", error: update?.error || "" });
        }
      };
      ...
      const renderRemoteStatus = (value, update = null) => {
        ...
        renderRemoteUpgrade(update || remote.update || null, true);
    ...
    function renderBoard(options = {}) {
      ...
      const boardMarkup = visibleStatuses.map(([status, statusLabel]) => {
        ...
        const cards = issues.map(issue => {
          ...
          const executionState = issue.status === "done" ? "completed" : issue.status === "cancelled" ? "interrupted" : issue.status === "blocked" ? "blocked" : issue.status === "in_review" ? (issue.latest_scheduler_status === "failed" ? "scheduler-failed" : "in_review") : replyResultState || (latestRunStatus === "failed" ? "failed" : latestRunStatus === "interrupted" ? "interrupted" : latestRunStatus === "scheduling" ? "scheduling" : latestRunStatus === "running" ? "running" : latestRunStatus === "claimed" ? "claimed" : issue.agent_enabled ? "not-started" : "");
    ...
    function openEditor(issue = null, initialStatus = "todo", createMode = "agent") {
      ...
      function conversationStatusMarkup(replyStatus) {
        ...
        const activityState = enrichmentLocked ? "thinking" : issue?.session_status === "stopping" ? "stopping" : issue?.session_retry ? "retrying" : relayFailure ? "relay-failed" : activeExecutionState || (issue?.enrichment_status === "failed" ? "title-regeneration-failed" : replyResultState || executionState);
        if (!activityState) return "";
        const activityLabel = activityState === "retrying" ? sessionRetryCardLabel(issue.session_retry) : t(issue?.enrichment_status === "regenerating" ? "标题生成中" : activityState === "title-regeneration-failed" ? "标题生成失败" : activityState === "stopping" ? "正在停止…" : activityState === "relay-failed" ? "Codex 会话连接失败" : activityState === "running" ? "工作中" : activityState === "scheduling" ? "调度中" : activityState === "scheduler-failed" ? "调度失败" : activityState === "claimed" ? "排队中" : activityState === "in_review" ? "待审核" : activityState === "completed" ? "已完成" : activityState === "blocked" ? "已阻塞" : activityState === "failed" ? "执行失败" : activityState === "interrupted" ? "已停止" : activityState === "not-started" ? "未开始" : "");
      ...
      function renderDialog() {
        ...
        if (draft.mode === "agent") {
          ...
          const hint = humanAssigned
            ...
            : t("创建后由 " + selectedName + " 立即开始工作，同时在后台生成 Issue 标题。");
        ...
        dialog.querySelectorAll("[data-dialog-select-option]").forEach(option => option.addEventListener("click", event => {
          ...
          if (name === "agent_id") {
            ...
            if (hint) hint.textContent = t("创建后由 " + selectedName + " 立即开始工作，同时在后台生成 Issue 标题。");
          }
          closeDialogSelects();
        ...
        const handleDialogOpenThread = event => {
          ...
          void (async () => {
            try {
              const opened = await openThread(threadId);
              if (opened.opened) dialog.close();
            } catch (error) {
              showError(error);
              if (errorOutput) {
