...
2. 打开 `更多` → `远程访问`。
3. 确认页面的 VPS 更新通道为`预览版`，然后在服务版本旁点击`检查升级`或`升级`。本机“远程访问”页也会显示 VPS 通道和检测结果，通过`访问网站`进入远程更新管理。
...
Beta 安装器会保存 `preview` 通道；已有明确配置会保留。旧 Beta 部署可能仍使用 Compose 默认的 Stable 通道，此时只能检测正式版。需要订阅 Beta 时，将 VPS 的 `deploy/hub/.env` 中 `BETTER_CODEX_RELAY_UPDATE_CHANNEL` 设置为 `preview`，使用原有 Compose 文件重新创建 Relay 容器；启用自动更新时，这会触发 Beta 检查和自动安装。如果页面提示无法在线安装或者在线升级失败，请使用上一节的精确版本 VPS 命令。自托管升级流程会保留部署模式和密钥，创建数据库备份，检查目标版本，并在验证失败时恢复上一版本及更新通道配置。
