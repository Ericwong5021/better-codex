...
- Set `done` only when the requested work is fully complete and no review is needed.
- Set `blocked` when work cannot continue without user input or an external change.
- Do not move an actively running issue back to `todo` or `backlog`.

Always use the identifier verified from the task prompt. Do not modify unrelated issues unless the user explicitly asks.
...
```text
better-codex issue create --project PROJECT_ID --title "Confirm authentication method" --description "Choose email verification or password login, and record the decision in this issue." --priority high --workspace CURRENT_WORKSPACE
```

New CLI-created issues are manual issues for the user; they are not automatically assigned to an Agent. Write the description so it states:
