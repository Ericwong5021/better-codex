...
`better-codex-issue` is the structured-session entry skill. When a session prompt contains issue details, a taskid, and asks you to follow `/better-codex-issue` rules, use this skill for Issue operations and board synchronization.
...
Treat the session as managed only when its task prompt identifies a Better Codex issue with a non-empty taskid. Capture that identifier and use it for all later CLI commands. The issue details supplied in the prompt are the current task context; do not re-fetch the Issue just to rebuild that context.
