...
The bundled Compose deployment enables `BETTER_CODEX_RELAY_AUTO_UPDATE=1` and defaults to `BETTER_CODEX_RELAY_UPDATE_CHANNEL=stable`. The verified installer persists the selected version's channel in `deploy/hub/.env`: `preview` for Beta and `stable` for a formal release. Installation and upgrade preserve an existing explicit channel and other deployment settings; an explicit `BETTER_CODEX_RELAY_UPDATE_CHANNEL` environment override changes the saved selection. Upgrades also snapshot this configuration and restore it on rollback. Set automatic updates to `0` to use manual updates. Apply environment changes by recreating the Relay container with the deployment's existing Compose files.

Legacy Beta deployments without a saved channel use the Compose stable default and therefore do not discover newer Betas. Set `BETTER_CODEX_RELAY_UPDATE_CHANNEL=preview` in their deployment environment to follow Betas. Changing a live deployment's channel with automatic updates enabled can trigger installation after the container restarts. The Remote access panel shows the VPS channel and cached detection result in desktop, local WebUI, and remote WebUI; local surfaces link to the remote website for authenticated update management.
