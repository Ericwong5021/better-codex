...
test("Hub backup and restore preserve paired devices, password state, and pending commands", () => {
  ...
  try {
    ...
    const timestamp = new Date().toISOString();
    const project: ProjectProjection = { id: "project-1", name: "Backup", identifier_prefix: "BKP", root_paths: [], description: "", overview_html: "", overview_status: "idle", overview_error: null, overview_updated_at: null, created_at: timestamp, updated_at: timestamp, local_revision: 1 };
...
test("Hub migration creates an integrity-checked backup before changing an older schema", () => {
  ...
  const backups = readdirSync(join(directory, "backups")).filter(name => name.startsWith("before-hub-v9-") && name.endsWith(".db"));
...
test("Hub migration archives legacy cancelled projections and rejects obsolete commands", () => {
  ...
  try {
    ...
    store.db.prepare("DELETE FROM hub_migrations WHERE version IN (5, 6, 7, 8, 9)").run();
    store.close();
    store = undefined;
...
test("Hub rejects newer schemas before opening or restoring them", () => {
  ...
  newer.prepare("INSERT INTO hub_migrations (version, applied_at) VALUES (10, ?)").run(new Date().toISOString());
  newer.close();
  try {
